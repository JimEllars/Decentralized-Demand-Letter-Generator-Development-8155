...
describe('usePdfGenerator', () => {
    ...
    it('blocks download and shows error if token is present but invalid when isPaid is true', async () => {
        const { result } = renderHook(() => usePdfGenerator(), { wrapper });
        ...
        const toast = await screen.findByText('Invalid payment token. Please complete payment again.');
        assert.ok(toast);
    });
    ...
    it('generates pdf successfully and shows success toast', async () => {
        ...
        // Using a properly structured base64-encoded mock JWT with Base64URL characters
        const validPayloadUrlEncoded = 'eyJ2YWxpZCI6dHJ1ZSwiX2lkIjoxMjM0NTZ9';
        // DO NOT USE BACKTICKS INSIDE STRING INTERPOLATION OR AST WILL BREAK esbuild
        globalThis.window.sessionStorage.setItem('axim_access_token', 'header.' + validPayloadUrlEncoded + '.signature');
        globalThis.window.sessionStorage.setItem('axim_token_expiry', new Date(Date.now() + 3600000).toISOString());

        const onError = mock.fn();
        const formData = { jurisdiction: 'CA', items: [{ description: 'Test', amount: 100 }] };
    ...
    it('generates pdf successfully with dev token', async () => {
        const { result } = renderHook(() => usePdfGenerator(), { wrapper });

        globalThis.window.sessionStorage.setItem('axim_access_token', 'dev-token-mock');
        globalThis.window.sessionStorage.setItem('axim_token_expiry', new Date(Date.now() + 3600000).toISOString());
